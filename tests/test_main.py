...
class TestStoreEndpoints(TestCase):
    ...
    def test_if_add_new_product_adds_a_new_product_to_json_list(self):
        ...
        self.assertEqual(response.status_code, 200, msg="Product added successfully")

    def test_if_create_a_sale_adds_a_new_sale_record(self):
        response = self.client.post('/api/v1/sales', data=json.dumps(self.sales), content_type='application/json')
        self.assertEqual(response.status_code, 200, msg="Sale record successfully added")

    def test_if_update_product_updates_details_for_a_specific_product(self):
        response = self.client.put('/api/v1/products/1', data=json.dumps(self.products), content_type='application/json')
        self.assertEqual(response.status_code, 200, msg="Product successfully Edited and Updated")
